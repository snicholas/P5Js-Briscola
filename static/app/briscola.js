...
var statusesv2 = ['indeck', 'hand', 'p1played', 'p2played',
    'p1taken', 'p2taken'
];
var briscola = null;
var p1, p2;
...
function setup() {
    ...
    for (j = 0; j < 4; j++) {
        for (i = 1; i <= 10; i++) {
            ...
            statodeckp1.push(statuses[0]);
            statodeckp2.push(statuses[0]);
        }
    }
    shuffle(deck, true);
    briscola = deck[38];
    ...
    for (var i = 0; i < p1.cards.length; i++) {
        statodeck[p1.cards[i].id - 1] = statusesv2[1];
        statodeckp1[p1.cards[i].id - 1] = statusesv2[1];
    }
    for (var i = 0; i < p2.cards.length; i++) {
        statodeck[p2.cards[i].id - 1] = statusesv2[3];
        statodeckp2[p2.cards[i].id - 1] = statusesv2[1];
    }
}
        ...
        for (var i = 0; i < p1.cards.length; i++) {
            if (p1.cards[i].x < mouseX && mouseX < p1.cards[i].x + 50) {
                if (p1.cards[i].y < mouseY && mouseY < p1.cards[i].y + 90) {
                    ...
                    statodeckp1[p1.cards[i].id - 1] = statusesv2[2];
                    statodeckp2[p1.cards[i].id - 1] = statusesv2[3];
                }
            }
        }
    }
        ...
        for (var i = 0; i < p2.cards.length; i++) {
            if (p2.cards[i].x < mouseX && mouseX < p2.cards[i].x + 50) {
                if (p2.cards[i].y < mouseY && mouseY < p2.cards[i].y + 90) {
                    ...
                    p2.selectedCard = p2.cards[i];
                    p2Played = p2.selectedCard;
                    p2.cards[i].chosen = true;
                    played++;
                    curPlayer = p1.id;
                    statodeck[p2.cards[i].id - 1] = statuses[4];
                    statodeckp1[p2.cards[i].id - 1] = statusesv2[3];
                    statodeckp2[p2.cards[i].id - 1] = statusesv2[2];
                }
            }
        }
    }
            ...
            if (played >= 2) {
                ...
                if (deck.length == 0 && p1.cards.length == 0 && p2.cards.length == 0) {
                    ...
                    if (curPlayer === p1.id) {
                        ...
                        statodeckp1[c[0].id - 1] = statusesv2[1];
                        statodeckp2[c[1].id - 1] = statusesv2[1];
                    }
                    if (curPlayer === p2.id) {
                        p2.cards.push(c[0]);
                        p1.cards.push(c[1]);
                        statodeckp1[c[1].id - 1] = statusesv2[1];
                        statodeckp2[c[0].id - 1] = statusesv2[1];
                    }
                    deck = deck.slice(2);
                }
            } else {
                if (curPlayer === p1.id && !p1.ishuman && !p1.isthinking) {
                    ...
                    p2.playCard(statodeckp2,briscola.seme)
                } else if (curPlayer === p2.id && !p2.ishuman && p2.selectedCard) {
                    p2Played = p2.selectedCard;
                    statodeck[p2.selectedCard.id - 1] = statuses[4];
                    statodeckp1[p2.selectedCard.id - 1] = statusesv2[3];
                    statodeckp2[p2.selectedCard.id - 1] = statusesv2[2];
                    p2.selectedCard = null;
                    p2.isthinking = false;
                    played++;
                    curPlayer = p1.id;
...
function winner(c1, c2) {
    ...
    if (c1.seme == briscola.seme && c2.seme != briscola.seme) {
        ...
        statodeckp1[c1.id - 1] = statusesv2[4];
        statodeckp1[c2.id - 1] = statusesv2[4];
        statodeckp2[c1.id - 1] = statusesv2[5];
        statodeckp2[c2.id - 1] = statusesv2[5];
        curPlayer = p1.id;
        ...
        statodeckp1[c1.id - 1] = statusesv2[5];
        statodeckp1[c2.id - 1] = statusesv2[5];
